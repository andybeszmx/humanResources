    ...
    <div class="contLeft">
      <div class="topTit">
        ...
        <div class="info">
          ...
          <p class="time">
            入职时间: {{ formatDate(user.timeOfEntry) }} 最新工资：{{ formLabelAlign.currentPostWage + formLabelAlign.currentBasicSalary }}
            <ul><span class="more">?</span><li>员工所有调薪后的基本工资、岗位工资合计</li></ul>
          </p>
        ...
        <div class="formTable">
          <el-form label-position="left" label-width="180px" :model="formLabelAlign">
            <el-form-item label="实际出勤天数（正式）：" style="width: 50%;">
              <el-input v-model="atteData.actualAtteOfficialDays" placeholder="0" disabled />
            </el-form-item>
            <el-form-item label="计薪天数（正式）：" style="width: 50%;">
              <el-input v-model="atteData.workingDays" placeholder="0" disabled />
            </el-form-item>
          </el-form>
        </div>
      </div>
    ...
    <div class="contRit">
      ...
      <div class="Items">
        <li v-for="(item, idx) in 2" :key="idx">
          <div class="name"><p>HR专员</p><p>2018-12-3 3:30</p></div>
...
const atteData = ref({})
const paymentData = reactive({ p1: 0, p2: 0, p3: 0, p4: 0 })
const formLabelAlign = reactive({})

const dutyStatusTxt = computed(() => user.value.inServiceStatus === 1 ? '在职' : '离职')
const dutyStatus = computed(() => user.value.inServiceStatus === 1 ? 'job-txt-green' : 'job-txt-red')
...
const calMoney = (type, money, days) => {
  if (type === 3) return money
  if (type === 1) return money * days
  return 0
}

const init = async () => {
  const archiveRes = await getAtteArchiveDetail({ userId, yearMonth })
  atteData.value = archiveRes || {}
  const settingsRes = await getSettings()
  paymentData.p1 = calMoney(settingsRes.housingSubsidyScheme, settingsRes.housingSubsidyAmount, atteData.value.actualAtteOfficialDays)
  paymentData.p2 = calMoney(settingsRes.transportationSubsidyScheme, settingsRes.transportationSubsidyAmount, atteData.value.actualAtteOfficialDays)
  paymentData.p3 = calMoney(settingsRes.communicationSubsidyScheme, settingsRes.communicationSubsidyAmount, atteData.value.actualAtteOfficialDays)
  paymentData.p4 = calMoney(settingsRes.lunchAllowanceScheme, settingsRes.lunchAllowanceAmount, atteData.value.actualAtteOfficialDays)

  const salaryDetail = await getSalaryDetail(userId)
  Object.assign(formLabelAlign, salaryDetail)

  const history = await getHistorysData({ userId, yearMonth })
  socialData.value = history
