...
const fetchArchivingList = async () => {
  loading.value = true
  tableData.value = await getArchivingList({ year: yearVal.value })
  loading.value = false
}

const openTable = async (obj, index) => {
  if (!obj.act) {
    const data = await getArchivingCont({ month: obj.yearsMonth, year: yearVal.value, opType: 2 })
    tableData.value[index].contentData = data
